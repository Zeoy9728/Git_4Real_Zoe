...
import type { PreviewData, ImageData } from "@/lib/types/file";

type ImagePreviewProps = {
  tweet?: boolean;
  viewTweet?: boolean;
  previewCount: number;
  imagesPreview: PreviewData;
      ...
      <AnimatePresence mode="popLayout">
        {imagesPreview.map(({ id, src, alt }, index) => (
          ...
          >
            <img
              className={cn(
                "relative h-full w-full cursor-pointer object-cover transition hover:brightness-75 hover:duration-200",
                isTweet
                  ? postImageBorderRadius[previewCount][index]
                  : "rounded-2xl"
              )}
            ...
            {removeImage && (
              <div
                className="group absolute left-0 top-0 translate-x-1 translate-y-1 rounded-full bg-light-primary/75 p-1 backdrop-blur-sm hover:bg-image-preview-hover/75"
